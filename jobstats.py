    ...
    def get_job_stats(self):
        ...
        if self.gpus:
            if self.gpu_vendor == "NVIDIA":
                ...
                self.get_data('gpu_total_memory', "max by(gpu_id, instance) (max_over_time(amd_gpu_total_vram{cluster='%s', job_id='%s'}[%ds:]))")
                self.get_data('gpu_used_memory', "max by(gpu_id, instance) (max_over_time(amd_gpu_used_vram{cluster='%s', job_id='%s'}[%ds:]))")
                self.get_data('gpu_utilization', "avg by(gpu_id, instance) (avg_over_time(amd_gpu_gfx_activity{cluster='%s', job_id='%s'}[%ds:]))")
